...
/*Görev 1: macSkoru()
  ...
  1. skor1 ve skor2 arasındaki fark nedir?
        skor1 fonksiyonun içinde bir değişken tanımlamış ve o değişkeni sıfıra eşitlemiş. Bu durumda 
        skor1 her çağırıldığında skor 0 a eşitlenecektir.
        skor2 fonksiyonu için ise skor değişkeni fonksiyonun dışında tanımlanmış ve sıfıra eşitlenmiştir.
        Bu durumda skor2 fonksiyonu her çağırıldığında skor değişkeni bir artacaktır.

  2. Hangisi bir closure kullanmaktadır? Nasıl tarif edebilirsin? (yarınki derste öğreneceksin :) )
        skor1 de closure vardır. 
  3. Hangi durumda skor1 tercih edilebilir? Hangi durumda skor2 daha mantıklıdır?
        Eğer değişkene dışarıdan erişmek durumunda isek skor2 değilsek skor1 daha mantıklıdır. 
*/
...
function takimSkoru() {
  ...
  const scor =  Math.floor(Math.random() * 15 +1);
  return scor;
}

console.log("team score: " + takimSkoru());
...
function macSonucu(callbackFnc, quarter) {
  let result = {
    EvSahibi: 0,
    KonukTakim:0,
  };
  for(let i= 0 ; i<=quarter ; i++){
    result.EvSahibi+=callbackFnc()
    result.KonukTakim+=callbackFnc()
  }
  return result;
...
function periyotSkoru(callbackFnc) {
  let result = {
    EvSahibi: callbackFnc(),
    KonukTakim:callbackFnc() ,
  };
  return result
}
